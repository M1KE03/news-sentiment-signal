...
*(Written before any result. Every decision below was fixed before estimation; any deviation is logged with its date in the [decision log](../docs/archive/research-review-decision-log.md).)*
...
- **Inference (D10–D12):** Newey–West `L = 5`, prespecified as one trading week and counted in **exchange sessions** ([spacing decision](../docs/archive/hac-spacing-decision.md)). One **primary** test — FinBERT, h = 1, unadjusted — and a closed secondary family of the remaining 14 (scorer, horizon) pairs under BH at q = 0.05 with Benjamini–Yekutieli alongside. The timing diagnostic is an exhaustive **circular shift** reported as a percentile rank.
...
**Figure 1.** Confusion matrices, three scorers side by side — and they carry the finding. In LM's true-positive row, **159 of 195 genuinely positive headlines are called neutral**: 81.5%. FinBERT's same row is 100 positive, 72 neutral, 23 negative.
