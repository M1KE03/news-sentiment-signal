...
### Latest: R13a — Act 1 measured. Both acts complete.

**R13a complete. Every increment in the repair sequence is now done and both acts are estimated.** 546 tests pass, 0 skip.

**800 headlines labelled by hand** under rubric v3 — 200 calibration, 600 evaluation — with 0 blanks, 0 invalid values, 0 text mismatches against the frozen blind export, and presentation order preserved in both files.

**The primary contrast is the one interval in this study that excludes zero.**

> `macroF1(FinBERT) − macroF1(LM) = +0.102, 95% pointwise [+0.050, +0.153]`, paired bootstrap over 595 article groups, n = 596.

Reported on both sets as §6a requires: full 596 gives +0.102, the confident subset (`hard = 0`, n = 541) gives **+0.104**. They agree to 0.002, so **the gap does not come from headlines a careful human could not resolve** — which is exactly the question the two-set split was prespecified to answer.
...
**The whole gap is one class, and the cause is mechanical rather than linguistic.** FinBERT and LM sit within 0.02 on negative and neutral. LM calls **159 of 195 genuinely positive headlines neutral (83%)**. The calibration diagnostic had already shown why: LM takes only **three distinct values** on headline-length text (−1, 0, +1), 78.3% of them exactly 0, because its score is `(pos − neg)/(pos + neg)` over dictionary matches and a short headline usually contains either no sentiment word or words of one sign. The Loughran–McDonald lists carry **2,345 negative terms against 347 positive** — an asymmetry built for 10-K risk language. Applied to headlines it collapses the positive class, and macro-F1 makes that visible where accuracy does not.

The report therefore does **not** say "the transformer reads financial language better". It says a document-level risk dictionary has almost no positive vocabulary to fire on at headline length.

**Accuracy is the weaker claim and is written as one:** FinBERT − LM accuracy is **+0.049, 95% [0.000, 0.097]** — lower bound exactly at zero. Accuracy is dominated by the 49% neutral majority. Where the two metrics differ in strength the report says so rather than quoting the larger.

**Supplementary McNemar disagrees and is shown anyway:** statistic 95.0, **p = 0.058**. Its independence assumption is contradicted by this design's own article-group structure (M4), so the bootstrap is primary and McNemar is not used as a tie-breaker.

**The two acts do not connect, and the appendix derived that in advance.** Act 1 separates the scorers cleanly; Act 2 cannot separate them at all (`delta = −2.43 bps [−6.88, +2.03]`). R12 predicted this before either was run: standardization puts the attenuation exponent at ½, and averaging a median of 337 headlines per session compresses a **twofold** per-headline noise difference into a **1.034×** coefficient difference. A classification gap of this size was never going to survive daily aggregation. The conditional hypothesis is therefore neither confirmed nor refuted — the design cannot test it at this aggregation, which is a stronger and more useful statement than a bare null.

**Three tests were retargeted rather than deleted**, because the state they guarded stopped existing: the provenance template must now *validate* rather than be *rejected*; the worksheets must be *complete* rather than *blank*; and the documents must now quote *exactly* the published macro-F1 values rather than claim no result. A fourth test was added requiring the confounds — the positive-class explanation and the 2,345/347 asymmetry — to appear wherever the +0.102 is quoted.

### Previous: R15 — the report is written against the outputs
...
| ✅ R13a — empirical validation | **Done 2026-09-10.** 800 headlines labelled under rubric v3; thresholds fitted on the 200 calibration items only and frozen in `config.VALIDATION_THRESHOLDS` before the evaluation set was scored; FinBERT's class taken as argmax, never a threshold on the tone scalar (B12) | **macroF1(FinBERT) − macroF1(LM) = +0.102 [+0.050, +0.153]**, n = 596 over 595 groups; +0.104 on the `hard = 0` subset. Class balance, 4 unusable, McNemar's disagreement (p = 0.058) and its validity condition, the single-annotator limitation and the absent kappa all reported. LM's threshold band recorded as one of 1,600 tied pairs, all classifying identically | B24; R06/R11 |
...
**Counting note (R09, 2026-09-10).** The table below lists **29** increments; prose in earlier checkpoints said "thirty", which was never true of the table and is corrected throughout. Nine rows completed in earlier sessions carried no tick, so the table disagreed with its own checkpoints; they are marked now. **29 of 29 complete, including R13a's empirical half.** Both acts are estimated. No increment remains.
